<template>
  <UCard>
    ...
    <template v-if="!isSoldOut">
      ...
      <div
        v-else
        class="flex items-center gap-2 rounded-2xl border border-sand-100 bg-sand-50/80 px-4 py-3 text-sm text-sand-600"
      >
        <UIcon
      ...
      >
        <div>
          ...
          <UFormGroup label="Number of seats" name="seats" required>
            <UInput
            ...
            <template #hint>
              <span class="text-xs text-sand-500">
                Up to {{ BOOKING_CONFIG.MAX_SEATS_PER_BOOKING }} seats per booking ({{ maxBookableSeats }}
                available)
              </span>
            </template>
          </UFormGroup>
        ...
        <div
          class="rounded-2xl border border-sand-100 bg-sand-50/70 px-5 py-4"
        >
          <p class="text-sm text-sand-500">Trip total</p>
          <p class="mt-1 text-3xl font-semibold text-sand-900">
            €{{ totalAmount }}
          </p>
        </div>
...
import type { ReserveBookingDto, ApiResponse, BookingSerialized, BookingReserveMeta } from '@booking/shared'
import { z } from 'zod'
  ...
  if (props.handleApiCall) {
    try {
      ...
      toast.add({
        ...
        description: `Your booking expires on ${new Date(response.meta.expiresAt).toLocaleString('en-US')}`,
        color: 'success',
      })
